...
import { sidebar_items } from "../../../data/Aside";
import BarItems from "./BarItems";
import { Link } from "react-router-dom";

const Aside = () => {
  return (
    <aside className="w-[280px] justify-between h-[100vh] p-[15px]  border-r border-[#E4E4E7]">
      <div className="flex mb-[56px] text-[#0A112F] m-[15px] font-bold text-[24px]">
        <img className="mr-[8px]" src={logo} alt="" />
        <h1>Payrole</h1>
      </div>
      ...
      <section>
        <ul>
          {sidebar_items.map((item) => (
            <BarItems key={item.id} item={item} />
          ))}
        </ul>
      </section>
      <Link to={"/setting"}>
        <div className="fixed bottom-5 flex ml-[15px] font-medium text-[16px]  text-[#9096A2]">
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
          ...
          <p className="ml-[15px]">Settings</p>
        </div>
      </Link>
    </aside>
  );
